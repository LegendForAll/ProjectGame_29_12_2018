...
void CMario::SetState(int state)
{
	...
	case SM_STAND_STATIC:
	{
		...
		else if ((IsKeyPress(DIK_A)) && isAttack == false)		//sm attack RIGHT/LEFT
		{
		...
		//-------------------------------------------------------------------
		else if (IsKeyPress(DIK_S) && isGround == true)			//sm jump
		{
			isGround = false;
			vy = -SM_JUMP_SPEED_Y;
			vx = 0;					//tam thoi = 0
	...
	case SM_MOVE_LEFT:
	{
		...
		else if (IsKeyPress(DIK_A) && isAttack == false)				//move - attack - stop move
		{
			Sound::GetInstance()->Play(USING_WHIP);
			vx = 0;
			this->state = SM_STAND_ATTACK;
		...
		//----------------------------
		else if (IsKeyPress(DIK_S) && isGround == true)
		{
			isGround = false;
			vy = -SM_JUMP_SPEED_Y;
			this->state = SM_JUMP;
	...
	case SM_SIT:
	{
		...
		else if (IsKeyPress(DIK_A) && isAttack == false)			//sm sit attack LEFT/RIGHT
		{
			Sound::GetInstance()->Play(USING_WHIP);
